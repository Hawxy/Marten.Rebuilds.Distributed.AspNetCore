...
{
    private static readonly FrozenSet<string> RebuildBlockedMethods = [HttpMethod.Post.Method, HttpMethod.Delete.Method, HttpMethod.Patch.Method, HttpMethod.Put.Method];
    ...
    {
        if (context.Request.Path != RebuildRoute && RebuildBlockedMethods.Contains(context.Request.Method) && await rebuildService.IsRebuilding())
        {
